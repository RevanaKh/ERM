  ...
  return (
    <>
      {!verifyotp && !resetPassword ? (
        <form className="max-w-md mx-auto  p-8 rounded-2xl space-y-6" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="w-full py-3 bg-[#75D6CB] text-white font-semibold rounded-xl hover:bg-[#047481] transition duration-300">
            {loading ? 'Mengirim...' : 'Kirim OTP'}
          </button>
        </form>
      ) : verifyotp && !resetPassword ? (
        <form onSubmit={handleVerify} className="max-w-md mx-auto  p-8 rounded-2xl  space-y-6">
      ...
      {error ? <p className="text-sm text-red-500 text-center">{error}</p> : message ? <p className="text-sm text-black text-center">{message}</p> : null}
    </>
  );
};
