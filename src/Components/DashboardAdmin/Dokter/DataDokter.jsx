...
const DataDokter = () => {
  ...
  const exportJadwalDokterToPDF = () => {
    ...
    const rows = jadwaldokter.map((jadwal) => [jadwal.nama, jadwal.poli, jadwal.hari, jadwal.jam_mulai, jadwal.jam_selesai, jadwal.status]);
    ...
    <>
      <div className="min-h-screen py-6 w-full bg-[#FAF7F2] ">
        ...
        <div className="bg-white shadow-lg flex flex-col px-5 justify-center items-center to-red-500 w-full min-h-[100px] ">
          <input type="text" placeholder="Cari Nama Dokter..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="border border-gray-300 rounded-lg mt-4 px-4 py-2 w-full " />
          <div class="relative overflow-x-auto shadow-lg my-[20px] w-full mx-[10px]">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {jadwaldokter.length === 0 ? (
                ...
                ) : jadwaldokter.filter((data) => data.nama.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 ? (
                  <tr>
                  ...
                  jadwaldokter
                    .filter((jadwal) => jadwal.nama.toLowerCase().includes(searchTerm.toLowerCase()))
                    .map((jadwal) => (
