...
const exportPasienToPDF = () => {
  ...
  const tableRows = dataPasienFiltered.map(data => ([
    `RM-${data.id}`,
    data.nama,
    data.nik,
    data.jenis_kelamin,
  ...
  <div className="flex flex-col  justify-between md:flex-row gap-4 items-center w-full">
    ...
    <button
      ...
      className="bg-[#00B686] hover:bg-[#166534] text-white rounded-lg px-6 py-2 transition-all"
    >
      Cari
    </button>
  </div>
              ...
              <tr key={data.id} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4 flex gap-2">
                  <button
                    ...
                    className="text-[#00B686] hover:text-[#007f5f] text-lg"
                  >
                    <FaEdit />
                  </button>
                  <button
